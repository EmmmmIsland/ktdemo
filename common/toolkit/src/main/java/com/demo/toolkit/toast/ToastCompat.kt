...
import com.demo.toolkit.toast.SMToast.Companion.makeText
 ...
 */ private constructor(context: Context, val baseToast: Toast) :
    Toast(context) {

    fun setBadTokenListener(listener: BadTokenListener): ToastCompat {
        (view!!.context as SafeToastContext).setBadTokenListener(listener)
        return this
    }
    ...
    companion object {
        ...
        fun makeText(context: Context, text: CharSequence?, duration: Int): ToastCompat {
            ...
            val toast = SMToast.makeText(context, text, duration)
            setContext(toast.view!!, SafeToastContext(context, toast))
