...
import com.example.baseproject.base.NestedLifecycleFragment

abstract class NestedLifecycleFragment : Fragment() {
    ...
    private fun enqueueDispatchVisible() {
        handler.post { dispatchChildVisibleState(true) }
    }
    ...
    private val isParentInvisible: Boolean
        private get() {
            ...
            return if (parentFragment is NestedLifecycleFragment) {
    ...
    private fun dispatchChildVisibleState(visible: Boolean) {
        ...
        if (!fragments.isEmpty()) {
            for (child in fragments) {
                if (child is NestedLifecycleFragment && child.isAdded() && !child.isHidden() && child.getUserVisibleHint()) {
     ...
     */
    fun onFragmentVisible(firstVisible: Boolean) {}
